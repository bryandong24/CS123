...
class InverseKinematics(Node):
  ...
  def fr_leg_fk(self, theta):
      ...
      T_RF_0_1 = translation(0.07500, -0.0835, 0) @ rotation_x(1.57080) @ rotation_z(theta[0])
  ...
  def fl_leg_fk(self, theta):
      ...
      ################################################################################################
      T_LF_0_1 = translation(0.07500, 0.0835, 0) @ rotation_x(1.57080) @ rotation_z(-theta[0])
  ...
  def br_leg_fk(self, theta):
      ...
      # TODO: implement forward kinematics here
      ################################################################################################
      T_RB_0_1 = translation(-0.07500, -0.0725, 0) @ rotation_x(1.57080) @ rotation_z(theta[0])
  ...
  def lb_leg_fk(self, theta): ################################################################################################
      # TODO: implement forward kinematics here
      ################################################################################################
      T_LB_0_1 = translation(-0.07500, 0.0725, 0) @ rotation_x(1.57080) @ rotation_z(-theta[0])
  ...
  def interpolate_triangle(self, t, leg_index):
      ...
      phase_points = [0.0, cycle_time/8.0, cycle_time/4.0, 3.0*cycle_time/8.0, cycle_time/2.0, 3.0*cycle_time/4.0, cycle_time]
      num_phases = len(phase_points)-1


      # Define phase shifts for each leg
      phase_shifts = [0.0, cycle_time/2.0, cycle_time/2.0, 0.0]  # FR, FL, BR, BL


      # Apply phase shift
      t_shifted = (t + phase_shifts[leg_index]) % cycle_time
     
      # Get the triangle positions for the current leg
      leg_positions = self.ee_triangle_positions[leg_index]


      # Get position index from time
      def index_for_time(t, phase_points):
           if (t < 0):
               print("error! t less than 0")
           for i in range(len(phase_points)):
               if (t < phase_points[i]):
                   return i-1
           print("error!!!!! t greater than cycle duration")


      i1 = index_for_time(t_shifted, phase_points)
      i2 = (i1+1)%num_phases
      t1 = phase_points[i1]
      t2 = phase_points[i1+1]
     
      # for testing
      print(t, leg_index, i1)


      x = np.interp(t_shifted, [t1, t2], [leg_positions[i1][0], leg_positions[i2][0]])
      y = 0
      z = np.interp(t_shifted, [t1, t2], [leg_positions[i1][2], leg_positions[i2][2]])
  ...
  def cache_target_joint_positions(self):
      ...
      for leg_index in range(4):
          ...
          target_joint_positions = [0.0] * 3
          for t in np.arange(0, cycle_time, 0.02):
              #print(t, leg_index)
